...
export function DestinationsPageClient({
  ...
  const filteredDestinations = useMemo(() => {
    ...
    return filtered;
  }, [destinations, activeRegion, searchQuery]);
                  ...
                  <div className="relative aspect-[4/3] rounded-2xl overflow-hidden">
                    ...
                    <div className="absolute inset-0 p-5 flex flex-col justify-end">
                      <span className="text-terracotta-300 text-xs font-accent font-medium uppercase tracking-wider mb-1">
                        {regions[destination.region as Region]?.name || destination.region}
                      </span>
                      <h3 className="text-white text-xl font-heading mb-1">
                        {locale === 'fr' ? destination.name : destination.nameEn}
                      </h3>
                      {destination.partnerName && (
                        <p className="text-white/60 text-sm">{destination.partnerName}</p>
                      )}
                    </div>
      ...
      <section className="py-16 bg-sand-100">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-2xl font-heading text-gray-900 mb-4">
            {translations.cantFindDestination}
          </h2>
          <p className="text-gray-600 mb-6 max-w-xl mx-auto">
            {translations.cantFindDescription}
          </p>
