...
interface FooterProps {
  locale: string;
  footerLogoUrl?: string; // Footer logo (white version) from admin settings
  contactInfo?: {
    email: string;
    phone: string;
    address: string;
...
export function Footer({ locale, footerLogoUrl, contactInfo, socialLinks: socialLinksFromProps, translations }: FooterProps) {
  // Use dynamic footer logo from admin settings, fallback to default white logo
  const footerLogo = footerLogoUrl || '/images/logo-dmc-alliance-white.svg';
  const currentYear = new Date().getFullYear();

  // Contact info with defaults
  const contact = {
    email: contactInfo?.email || 'contact@dmc-alliance.org',
    phone: contactInfo?.phone || '+33 1 23 45 67 89',
    address: contactInfo?.address || 'Paris, France',
  };
  ...
  // Build social links from props, only include those that have URLs
  const socialLinksConfig = [
    {
      name: 'LinkedIn',
      href: socialLinksFromProps?.linkedin,
      icon: (
    ...
    {
      name: 'Instagram',
      href: socialLinksFromProps?.instagram,
      icon: (
        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    ...
    {
      name: 'Facebook',
      href: socialLinksFromProps?.facebook,
      icon: (
        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    ...
    {
      name: 'X',
      href: socialLinksFromProps?.twitter,
      icon: (
        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    ...
    {
      name: 'YouTube',
      href: socialLinksFromProps?.youtube,
      icon: (
        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
  ...
  // Filter to only show social links that have URLs
  const socialLinks = socialLinksConfig.filter(link => link.href);
      ...
      <div className="border-b border-gray-800">
        <div className="container mx-auto px-4 py-12">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <NewsletterForm locale={locale} variant="default" />
          </div>
        </div>
      </div>
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm text-gray-400">
              <li className="flex items-start gap-3">
                ...
                <a href={`mailto:${contact.email}`} className="hover:text-terracotta-400 transition-colors">
                  {contact.email}
                </a>
              </li>
              <li className="flex items-start gap-3">
                ...
                <a href={`tel:${contact.phone.replace(/\s/g, '')}`} className="hover:text-terracotta-400 transition-colors">
                  {contact.phone}
                </a>
              </li>
              <li className="flex items-start gap-3">
