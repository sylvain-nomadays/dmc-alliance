...
import { type Article, type categories as CategoriesType } from '@/data/articles';
...
function ArticleCard({
  ...
  const categoryColor = categoryColors[article.category] || categoryColors.destinations;
  ...
  const categoryData = categories[article.category];
              ...
              {article.author?.avatar ? (
                ...
                <div className="w-8 h-8 rounded-full bg-terracotta-100 flex items-center justify-center">
                  <span className="text-xs font-medium text-terracotta-600">
                    {(article.author?.name || 'DA').charAt(0)}
                  </span>
                </div>
              )}
              <span className="text-gray-700 font-medium truncate max-w-[120px]">
                {article.author?.name || 'DMC Alliance'}
              </span>
            </div>
            <span className="text-gray-400">{article.readTime} min</span>
          </div>
        </div>
      </div>
    </Link>
      ...
      {mainFeatured && activeCategory === 'all' && !searchQuery && (
        <section className="py-12 bg-sand-50">
          <div className="container mx-auto px-4">
            <ArticleCard article={mainFeatured} locale={locale} featured categories={categories} />
          </div>
        </section>
      )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredArticles
                ...
                .map((article, index) => (
                  ...
                  >
                    <ArticleCard article={article} locale={locale} categories={categories} />
                  </div>
                ))}
            </div>
          )}
      ...
      <section className="py-16 bg-terracotta-500">
        <div className="container mx-auto px-4">
          <div className="max-w-2xl mx-auto text-center">
            <h2 className="text-3xl font-heading text-white mb-4">{translations.newsletter.title}</h2>
            <p className="text-white/80 mb-8">{translations.newsletter.subtitle}</p>
