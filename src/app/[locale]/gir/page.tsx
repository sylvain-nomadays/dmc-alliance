...
import { circuits, type Circuit, type CircuitDeparture } from '@/data/circuits';
...
function GirContent({ locale }: { locale: string }) {
  ...
  // Get all GIR circuits
  const girCircuits = useMemo(() => circuits.filter(c => c.isGir), []);

  // Get unique destinations from GIR circuits
  const destinations = useMemo(() => {
    const destSlugs = [...new Set(girCircuits.map(c => c.destinationSlug))];
    return destSlugs.map(slug => {
      const circuit = girCircuits.find(c => c.destinationSlug === slug);
      const partner = partners.find(p => p.id === circuit?.partnerId);
      const destination = partner?.destinations.find(d => d.slug === slug);
      return {
        slug,
        name: destination?.name || slug,
        nameEn: destination?.nameEn || slug
      };
    });
  }, [girCircuits]);
  ...
  const filteredCircuits = useMemo(() => {
    let filtered = girCircuits;

    if (selectedDestination) {
      filtered = filtered.filter(c => c.destinationSlug === selectedDestination);
    }

    if (selectedMonth) {
      filtered = filtered.filter(c =>
        c.departures.some(d => d.startDate.startsWith(selectedMonth))
      );
    }

    return filtered;
  }, [girCircuits, selectedDestination, selectedMonth]);

  // Get upcoming departures count
  const upcomingDepartures = useMemo(() => {
    const today = new Date().toISOString().split('T')[0];
    return girCircuits.reduce((acc, circuit) => {
      return acc + circuit.departures.filter(d => d.startDate >= today && d.status !== 'full').length;
    }, 0);
  }, [girCircuits]);
...
function CircuitCard({ circuit, locale }: { circuit: Circuit; locale: string }) {
  const isFr = locale === 'fr';
  const partner = partners.find(p => p.id === circuit.partnerId);
  ...
  const getStatusBadge = (departure: CircuitDeparture) => {
      ...
      case 'few_spots':
        return (
          <span className="bg-orange-100 text-orange-700 text-xs font-medium px-2 py-0.5 rounded-full">
            {isFr ? `${departure.availableSpots} places` : `${departure.availableSpots} spots`}
          </span>
        );
      case 'full':
        return (
      ...
      default:
        return (
          <span className="bg-blue-100 text-blue-700 text-xs font-medium px-2 py-0.5 rounded-full">
            {departure.availableSpots} {isFr ? 'places' : 'spots'}
          </span>
        );
    }
  };
        ...
        <div className="md:w-2/3 p-6">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
            <div>
              <span className="text-sm text-terracotta-600 font-medium">
                {isFr ? destination?.name : destination?.nameEn} • {partner?.name}
              </span>
          ...
          <div className="flex flex-wrap gap-4 mb-4 text-sm text-gray-600">
            <div className="flex items-center gap-1">
              ...
              {circuit.duration.days} {isFr ? 'jours' : 'days'} / {circuit.duration.nights} {isFr ? 'nuits' : 'nights'}
            </div>
            <div className="flex items-center gap-1">
              <UserGroupIcon className="w-4 h-4 text-gray-400" />
              {circuit.practicalInfo.groupSize.min}-{circuit.practicalInfo.groupSize.max} {isFr ? 'personnes' : 'people'}
            </div>
            <div className="flex items-center gap-1">
              <span className={cn(
                'px-2 py-0.5 rounded-full text-xs font-medium',
                circuit.difficulty === 'easy' && 'bg-green-100 text-green-700',
                circuit.difficulty === 'moderate' && 'bg-yellow-100 text-yellow-700',
                circuit.difficulty === 'challenging' && 'bg-orange-100 text-orange-700',
                circuit.difficulty === 'expert' && 'bg-red-100 text-red-700'
              )}>
                {circuit.difficulty === 'easy' && (isFr ? 'Facile' : 'Easy')}
                {circuit.difficulty === 'moderate' && (isFr ? 'Modéré' : 'Moderate')}
                {circuit.difficulty === 'challenging' && (isFr ? 'Sportif' : 'Challenging')}
                {circuit.difficulty === 'expert' && 'Expert'}
              </span>
            </div>
          </div>
          ...
          <div className="mb-6">
            <div className="flex flex-wrap gap-2">
              {(isFr ? circuit.highlights.fr : circuit.highlights.en).slice(0, 3).map((highlight, idx) => (
                <span key={idx} className="bg-sand-100 text-gray-700 text-xs px-3 py-1 rounded-full">
                  {highlight}
                </span>
              ))}
              {circuit.highlights.fr.length > 3 && (
